    ...
    methods: {
        async getUserDetailById() {
            ...
            if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
              // 有值 则表示已经有了一个上传成功的图片
              // 上传成功的图片 upload: true 表示该图片已经上传成功(该图片是从库里拿出来的)
        ...
        async saveUser() {
          ...
          // 判断  当前图片有没有上传完成
          if (fileList.some(item => !item.upload)) {  // fileList:[{url:'baidu.com', upload: true},{url:'google.com'}]
            // 说明此时 有图片还没有上传完成            // 遍历检测fileList中有没有不存在upload属性，有则return true
            this.$message.warning('此时还有图片未上传完成')
            return
          }
          // staffPhoto由于接口问题 必须给一个 有空格的字符串 才能存进去
